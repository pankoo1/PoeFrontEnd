...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children }) => {
  ...
  const userName = localStorage.getItem('nombre') || 'Administrador';

  const menuItems = [
    { icon: <LayoutDashboard size={20} />, label: 'Dashboard', path: '/dashboard' },
    { icon: <Users size={20} />, label: 'Usuarios', path: '/users' },
    { icon: <ClipboardList size={20} />, label: 'Tareas', path: '/admin-tareas' },
    ...
    <div className="flex h-screen bg-slate-50">
      ...
      <aside className="w-72 bg-slate-900 flex flex-col shadow-xl">
        ...
        <div className="p-6 border-b border-slate-800">
          ...
          <p className="text-slate-400 text-sm mt-1">Control Total del Sistema</p>
        </div>
